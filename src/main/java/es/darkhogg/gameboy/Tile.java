...
public final class Tile {
	...
	public static Tile fromBuffer(ByteBuffer buffer) {
		final var oldOrder = buffer.order();
		buffer.order(ByteOrder.BIG_ENDIAN);

		final var first = buffer.getLong();
		final var last = buffer.getLong();

		buffer.order(oldOrder);
		return new Tile(first, last);
	}
